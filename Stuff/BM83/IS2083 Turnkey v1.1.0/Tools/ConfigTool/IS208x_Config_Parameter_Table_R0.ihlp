...
When the One_Key_Operation is triggered, it enters CSB pairing as Slave role first and then change to Master role after this timeout occurs.
Unit 2s
;;help_end;;
...
If this option is enabled (set to 1), the speaker will try to recover the CSB link when the CSB link disconnects abnormally. Only for Stereo mode.
;;help_end;;
...
Enable seamless audio resync that can keep audio sync in CSB master and other slaves with audio breaks when a new slave is added.
;;help_end;;
ENABLE_SEAMLESS_AUDIO_RESYNC
Allow Voice Prompt in MSPK Slave
Set this option to allow voice prompt in MSPK slave(s). Please note that it might cause audio unsync after MSPK slave
;;help_end;;
