 ...
 * 
 * The `lv_disp_flush_ready()` function is called after the display has been flushed
 * 
 * @param disp The display driver to use.
 * @param area The area of the display to be flushed.
 * @param color_p A pointer to a color structure.
 */
...
*/
/**
 * Reads the music control buttons and sets the corresponding bits in the configuration register to send the according
 * command to the BM83 module
 * 
 * @param pvParameters The parameter is a pointer to the task handle.
 */
...
/**
 * When the user clicks on the back button, the previous tab is activated
 * 
...
/**
 * When the user clicks on a button, the tab view is set to the corresponding tab
 * 
 * @param obj The object that received the event.
...
/**
 * * * Standby Mode:
 *  When the Standby button is pressed, the 7.5V, +-5V, PVCC, and Power Amp shutdown activation pins
 * are set to low, and the Power Amp mute activation pin is set to high
 * 
 * @param btn_standby The button object that we want to attach the event to.
 * @param event The event that triggered the callback.
 */
...
/**
 * * Power down process:
 * * Set the power down pin to low.
 * * Set the power amp shutdown pin to low.
 * * Set the +-5V pin to high.
 * * Set the PVCC pin to high.
 * * Sets GPIO26 to ~3.1 V
 * * Set the +-5V pin to high.
 * * Set the 7.5V pin to high.
 * * Set the sound processor pins to the default values.
 * * Set the power down pin to low
 * * Set the power amp mute pin to low.
 * 
 * @param btn_pwrup The button object that we created in the previous step.
 * @param event The event that triggered the callback.
 */
...
/**
 * * Power down process:
 * * Set the power down pin to high.
 * * Set the power amp mute pin to high.
 * * Set the power amp shutdown pin to high.
 * * Set the +-5V pin to low.
 * * Set the PVCC pin to low.
 * * Set the DAC1 pin to 0.
 * * Set the +-5V pin to low.
 * * Set the 7.5V pin to low.
 * * Set the sound processor pins to the default values.
 * * Set the power down pin to low
 * 
 * @param btn_pwrdwn The button object that we created in the previous step.
 * @param event The event that triggered the callback.
 */
...
/**
 * * Set the sound processor to the default settings
 * 
 * @param btn_analog_on The button object that we created earlier.
 * @param event The event that triggered the callback.
 */
...
/**
 * * The function is called when the user clicks on the "Power amp on" button. 
 * * The function sets GPIO26 to ~3.1 V, which is the voltage that the power amp needs to turn on. 
 * * The function also enables the power amp by setting the PVCC pin to high. 
 * * The function also deactivates the power amp shutdown by setting the shutdown pin to low. 
 * * The function also deactivates the power amp mute by setting the mute pin to low. 
 * * The function also deactivates the sound processor mute by setting the mute pin to low. 
 * * The function ends by printing a message to the serial monitor.
 * 
 * @param btn_poweramp_on The button object that we want to attach the event to.
 * @param event The event that triggered the callback.
 */
...
/**
 * * When the BM83 button is pressed, the MFB pin is set to "High" to power on the BM83.
 * * The BM83 is then powered on by sending the "power on" command over UART.
 * * The MFB pin is then set to "Low". Setting the pin "High" is only required everytime the BM83 Module has to be setup again.
 * 
 * @param btn1 The button object that was clicked.
 * @param event The event that triggered the callback.
 */
...
/**
 * When the user clicks on the "Pair" button, the MMI action "Entering Pairing" is sent to the BM83
 * 
 * @param btn2 The button that was clicked.
 * @param event The event that triggered the callback.
 */
...
/**
 * * Set the sound setup to the default values
 * 
 * @param btn3 the button that was pressed
 * @param event 
 */
...
/**
 * When the state of the switch changes, print out the state of the switch and set the state of the
 * according MCP23017 pin
 * 
 * @param sw2 The switch object
 * @param event The event that triggered the callback.
 */
...
// on change
/**
 * * Rotate the rotary encoder and set the volume accordingly
 * 
 * @param u
 */
...
/**
 * When the encoder is turned, it will send a signal to the microcontroller. The microcontroller will
 * then read the signal and determine if the encoder has been turned. If it has, it will send a signal
 * to the microcontroller that will be read by the encoder_read function. The encoder_read function
 * will then determine if the encoder has been turned. 
 * 
 * @param encoder_indev The pointer to the encoder object.
...
/**
 * Get the current encoder count and the difference between the current count and the last count
 * 
 * @return The difference between the current encoder count and the previous encoder count.
 */
...
/**
 * * When the user switches to the music tab, the music control container is moved to the foreground
 * and the menu control container is moved to the background.
 * * When the user switches to the menu tab, the menu control container is moved to the foreground and
 * the music control container is moved to the background
 * 
 * @param pvParameters The parameter is a pointer to the task handle.
 */
...
/**
 * When the slider is moved, the slider value is read and the slider label is updated. The slider value
 * is then used to set the input gain
 * 
 * @param slider_ingain the slider object
 * @param event The event that triggered the callback.
 */
...
/**
 * When the slider value changes, update the label text and set the bass value
 * 
 * @param slider_bass the slider object
 * @param event The event that triggered the callback.
 */
...
/**
 * When the slider value changes, update the label text and set the middle value
 * 
 * @param slider_mid The slider object.
 * @param event The event that triggered the callback.
 */
...
/**
 * When the slider value changes, update the label text and set the treble value
 * 
 * @param slider_treb the slider object
